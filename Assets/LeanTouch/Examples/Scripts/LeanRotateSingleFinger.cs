...
namespace Lean.Touch
{
	...
	public class LeanRotateSingleFinger : MonoBehaviour
	{
		...
		public bool AllowPitchAndYawAtSameTime = false;
		public bool PreventCameraBelowZero = false;
		...
		protected virtual void Rotate(float twistDegrees)
		{
			...
			if (camera != null)
			{
				...
				if (IsCamera)
					transform.Rotate(Vector3.up * twistDegrees, Space.World);
				else
		...
		protected virtual void RotatePitch(float pitchDegrees)
		{
			...
			if (camera != null)
			{
				if (IsCamera)
				{					
					if (PreventCameraBelowZero && pitchDegrees > 0 || camera.transform.position.y > 0) //Block camera from going below zero
					{
						//transform.Rotate(Vector3.right * pitchDegrees, Space.Self);
						transform.Rotate(camera.transform.right * pitchDegrees, Space.World);
					}
                    else
                    {
						transform.Rotate(camera.transform.right * pitchDegrees, Space.World);
