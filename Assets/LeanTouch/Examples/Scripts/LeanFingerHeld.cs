	...
	public class LeanFingerHeld : MonoBehaviour
	{
		...
		private void OnFingerSet(LeanFinger finger)
		{
			...
			if (link != null)
			{
				...
				//editd by cwm - ignore movement when already set
				bool set;
				if(link.LastSet)
                {
					set = finger.Age >= MinimumAge;
				}
				else
                {
					set = finger.Age >= MinimumAge && link.TotalScaledDelta.magnitude < MaximumMovement;
				}

				//added cwm - cancel set if it has been longer than minium age - fixes bug where moving finger back to position engages pan
				if (finger.Age > MinimumAge && set == false)
				{
					links.Remove(link);
				}
